    ...
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" id="productForm">
        ...
        <div class="form-group" style="margin: 24px 0; padding: 16px; background: #f9fafb; border-radius: 8px;">
            <div class="checkbox-group">
                <input type="checkbox" name="has_variants" id="has_variants" value="1" {{ old('has_variants') ? 'checked' : '' }} onchange="toggleVariants()">
            ...
            <div class="addon-groups-list">
                ...
                <div class="addon-group-item" style="background: #fff; border: 1px solid #e5e7eb; border-radius: 8px; padding: 12px 16px; margin-bottom: 8px; display: flex; align-items: center; gap: 12px;">
                    <input type="checkbox" name="addon_groups[]" id="addon_group_{{ $group->id }}" value="{{ $group->id }}" 
                        {{ in_array($group->id, old('addon_groups', [])) ? 'checked' : '' }}
    ...
    const variantHtml = `
        <div class="variant-card" id="variant-${variantIndex}">
            <div class="variant-header">
                <span class="variant-title">Variant #${variantIndex + 1}</span>
            ...
            <div class="variant-grid">
                <div class="form-group">
                    <label>Name*</label>
                    <input type="text" name="variants[${variantIndex}][name]" value="${defaultName}" required placeholder="e.g., Small, Medium, Large">
                </div>
                <div class="form-group">
                    <label>Price*</label>
                    <input type="number" step="0.01" name="variants[${variantIndex}][price]" required min="0" placeholder="0.00">
                </div>
                <div class="form-group">
                    <label>Compare Price</label>
                    <input type="number" step="0.01" name="variants[${variantIndex}][compare_price]" min="0" placeholder="0.00">
                </div>
                <div class="form-group">
                    <label>Stock</label>
                    <input type="number" name="variants[${variantIndex}][stock_quantity]" min="0" value="0">
                </div>
                <div class="form-group">
                    <label>SKU</label>
                    <input type="text" name="variants[${variantIndex}][sku]" placeholder="Optional">
                </div>
            </div>
        </div>
    `;
