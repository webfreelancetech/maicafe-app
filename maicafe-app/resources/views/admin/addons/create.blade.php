...
@section('title', 'Create Addon Group - Mai Cafe Admin')

@section('content')
<div class="page-header">
    <h1>Create Addon Group</h1>
...
<div class="card">
    <form action="{{ route('admin.addons.store') }}" method="POST" id="addonForm">
        @csrf
        
        <div class="form-group">
            <label for="name">Group Name*</label>
            <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="e.g., Milk Options, Extra Toppings">
            @error('name')
        ...
        <div class="form-group">
            ...
            <input type="text" name="description" id="description" value="{{ old('description') }}" placeholder="e.g., Choose your preferred milk type">
        </div>
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <select name="selection_type" id="selection_type" required>
                    <option value="multiple" {{ old('selection_type') == 'multiple' ? 'selected' : '' }}>Multiple Select (Checkboxes)</option>
                    <option value="single" {{ old('selection_type') == 'single' ? 'selected' : '' }}>Single Select (Radio)</option>
                </select>
        ...
        <div class="form-grid" id="selectionLimits" style="display: none;">
            <div class="form-group">
                ...
                <input type="number" name="min_selections" id="min_selections" value="{{ old('min_selections', 0) }}" min="0">
            </div>

            <div class="form-group">
                <label for="max_selections">Maximum Selections</label>
                <input type="number" name="max_selections" id="max_selections" value="{{ old('max_selections') }}" min="1" placeholder="Leave empty for unlimited">
            </div>
        </div>
        ...
        <div style="margin-top: 24px;">
            <button class="btn btn-primary" type="submit">
                <i class="fas fa-save"></i> Create Addon Group
            </button>
        </div>
    </form>
</div>
...
function addAddonOption(defaultName = '', defaultPrice = '') {
        ...
        <div class="addon-option-card" id="option-${optionIndex}">
            <div class="option-fields">
                <div class="form-group">
                    ...
                    <input type="text" name="addons[${optionIndex}][name]" value="${defaultName}" required placeholder="e.g., Oat Milk, Extra Shot">
                </div>
                <div class="form-group">
                    <label>Additional Price (£)*</label>
                    <input type="number" step="0.01" name="addons[${optionIndex}][price]" value="${defaultPrice}" required min="0" placeholder="0.00">
                </div>
            </div>
